...
curl -H "x-api-key:${NEW_RELIC_API_KEY}" -d "deployment[app_name]=${NEW_RELIC_APP}" -d "deployment[description]=${NEW_RELIC_DESCRIPTION}" -d "deployment[revision]=${NEW_RELIC_REVISION}" -d "deployment[changelog]=${NEW_RELIC_CHANGELOG}" -d "deployment[user]=${CI_COMMITTER_USERNAME}" https://api.newrelic.com/deployments.xml
